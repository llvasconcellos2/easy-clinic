...
Template.settingsForm.onRendered(function () {
  $(document).ready(function () {
    ...
    // slotDuration
    var slotDurationInput = $("input[name=slotDuration]");
    if (slotDurationInput.length) {
      let apptGroup = $.parseHTML('<div class="input-group size-sm"></div>');
      slotDurationInput.parent().find("label").after(apptGroup);
      slotDurationInput.detach().appendTo(apptGroup);
    }
    ...
    if (apptInput.length) {
      ...
      var apptGroup = $.parseHTML('<div class="input-group size-sm"></div>');
