...
Template.schedule.onCreated(function () {
  ...
  templateInstance.doctors = Meteor.users
  ...
  AutoForm.addHooks("quickPatientForm", {
    onSuccess: function (formType, result) {
      toastr["success"](
        TAPi18n.__("common_save-success"),
        TAPi18n.__("common_success"),
      );
...
Template.schedule.onRendered(function () {
  ...
  var setAppointmentFormModalButtons = function () {
    ...
    $("#scheduleEventForm .delete-btn").click(function (event) {
      ...
      Schedule.remove(scheduleEvent, function (error, result) {
        ...
        if (result) {
          toastr["warning"](
            TAPi18n.__("schedule_appointment-canceled"),
            TAPi18n.__("common_success"),
          );
        }
      });
  ...
  var showEventModal = function (scheduleEvent) {
    ...
    $("#scheduleEventForm .scheduleTitle").html(
  ...
  var calendar = $("#calendar").fullCalendar({
    ...
    selectAllow: function (selectInfo) {
      ...
      if (doctor) {
        ...
        if (doctor.workHours) {
          ...
          toastr["error"](
            TAPi18n.__("schedule_no-doctor-workhours-error"),
            TAPi18n.__("common_notAllowed"),
          );
        }
      }
    },
        ...
        Schedule.insert(
          ...
          function (error, result) {
            ...
            if (error) {
              if (error.error == 403) {
                toastr["error"](
                  TAPi18n.__("schedule_notAllowedError"),
                  TAPi18n.__("common_notAllowed"),
                );
              } else {
                toastr["error"](error.message, TAPi18n.__("common_error"));
    ...
    eventRender: function (event, element, timelineView) {
      ...
      if (
        timelineView.name == "timelineDay" ||
        timelineView.name == "timelineThreeDays"
      ) {
        ...
        if (event.title == "to-confirm") {
          element
            .find(".fc-title")
            .html(TAPi18n.__("schedule_status-to-confirm"));
        }
      }
      element.qtip({
        position: {
    ...
    resourceRender: function (resource, labelTds, bodyTds, view) {
      ...
      if (resource.picture) {
        ...
        if (image) {
          ...
          pictureUrl = image.link().replace(/^https?:\/\/[^\/]+/i, "");
        }
      } else {
    ...
    if ($("#calendar").fullCalendar("getEventSources").length == 0) {
      $("#calendar").fullCalendar(
        ...
        function (start, end, timezone, callback) {
          var events = (self.templateInstance().events =
            Schedule.find().fetch());
          if (callback) {
            callback(events);
          }
